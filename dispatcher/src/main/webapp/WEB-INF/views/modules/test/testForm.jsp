...
<ul class="nav nav-tabs">
    ...
    <li class="active"><a href="${ctx}/test/test/form?id=${test.id}">组件<shiro:hasPermission
            name="test:test:edit">${not empty test.id?'修改':'表单'}</shiro:hasPermission><shiro:lacksPermission
            name="test:test:edit">查看</shiro:lacksPermission></a></li>
</ul>
<br/>
<form:form id="inputForm" modelAttribute="test" action="${ctx}/test/test/save" method="post" class="form-horizontal">
    ...
    <div class="control-group">
        ...
        <div class="controls">
            <form:radiobuttons path="name" items="${fns:getDictList('yes_no')}" itemLabel="label" itemValue="value"
                               htmlEscape="false"/>
        </div>
    </div>
    <div class="control-group">
        ...
        <div class="controls">
            <form:checkboxes path="name" items="${fns:getDictList('yes_no')}" itemLabel="label" itemValue="value"
                             htmlEscape="false"/>
        </div>
    </div>
    <div class="control-group">
        ...
        <div class="controls">
            <sys:treeselect id="user" name="user.id" value="${user.user.id}" labelName="user.name"
            ...
            <sys:treeselect id="user2" name="user.id" value="${user.user.id}" labelName="user.name"
                            labelValue="${user.office.name}"
                            title="用户" url="/sys/office/treeData?type=3" cssClass="required" notAllowSelectParent="true"
                            checked="true"/>
            <sys:treeselect id="office2" name="office.id" value="${user.office.id}" labelName="office.name"
                            labelValue="${user.office.name}"
                            title="部门" url="/sys/office/treeData?type=2" cssClass="required" notAllowSelectParent="true"
                            checked="true"/>
            <sys:treeselect id="company2" name="company.id" value="${user.company.id}" labelName="company.name"
                            labelValue="${user.company.name}"
    ...
    <div class="control-group">
        ...
        <div class="controls">
            <form:textarea id="remarks" htmlEscape="true" path="remarks" rows="4" maxlength="200"
                           class="input-xxlarge"/>
    ...
    <div class="form-actions">
        <shiro:hasPermission name="test:test:edit"><input id="btnSubmit" class="btn btn-primary" type="submit"
