...
(function ($) {
            ...
            if (options.isTip) {
                ...
                if (tip && tip.next == true) {
                    ...
                    if (top == $box.offset().top) {
                        ...
                        $box.find('#jbox-content').html(tip.options.content.substr(5)).end().css({left: (($window.width() - $box.outerWidth()) / 2)}).animate({
                            top: top,
                            opacity: 0.1
                        }, 500, removeBoxImpl);
                    }
        ...
        if (options.isMessager) {
            ...
            if (tip && tip.next == true) {
                ...
                if (!data.isObject && isRequest) {
                    ...
                    $box[showFunc](options.showSpeed, loaded);
                    /* loaded */
                }
            }
        }
    ...
    $.jBox.stateDefaults = {
        content: '', buttons: {'确定': 'ok'}, buttonsFocus: 0, submit: function (v, h, f) {
            return true;
        }
    };
    $.jBox.tipDefaults = {
